        ...
        for y, row in enumerate(self.grid):
            for x, value in enumerate(row):
                if value == 1:
                    ...
                    pygame.draw.rect(screen, (0, 255, 255), (x*block_size, y * block_size, block_size, block_size))
                elif value == 3:
...
def main():
    ...
    while running:
        ...
        while new_map == True or path == None:
            start = (random.randrange(1, grid_size[0]//2), random.randrange(1, grid_size[1]//2))
            goal = (random.randrange(grid_size[0]//2 + 1, grid_size[0] - 2), random.randrange(grid_size[1]//2 + 1, grid_size[1] - 2))
            grid = Grid(grid_size, cluster_size, start, goal)
            path = grid.astar_search(start, goal)
            new_map = False

        grid.draw_map(screen) # Draw the grid and path


        for pos in path:
            pygame.draw.rect(screen, (255, 0, 0), (pos[0] * 10 + 2, pos[1] * 10 + 2, 6, 6))
